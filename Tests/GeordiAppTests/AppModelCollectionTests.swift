...
struct AppModelCollectionTests {
  ...
  func invalidLinkedCacheFallsBackToSynthetic() throws {
    ...
    let store = UserDataStore(root: parent.appending(path: "UserData"))
  ...
  private func makeModel(
    ...
    store: UserDataStore? = nil,
    live: @escaping @Sendable () async throws -> GraphSnapshot
  ) -> AppModel {
    let resolvedStore: UserDataStore
    if let store {
      resolvedStore = store
    } else {
      let parent = FileManager.default.temporaryDirectory
        .appending(path: UUID().uuidString, directoryHint: .isDirectory)
      resolvedStore = UserDataStore(root: parent.appending(path: "UserData"))
    }
    return AppModel(
      configuration: .phaseZero,
      applicationClassifications: nil,
  ...
  private func temporaryStore(containing snapshot: GraphSnapshot) throws -> UserDataStore {
    let parent = FileManager.default.temporaryDirectory
      .appending(path: UUID().uuidString, directoryHint: .isDirectory)
    let store = UserDataStore(root: parent.appending(path: "UserData"))
