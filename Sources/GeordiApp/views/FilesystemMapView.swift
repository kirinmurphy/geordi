...
struct FilesystemMapView: View {
    ...
    ScrollView {
      VStack(alignment: .leading, spacing: 22) {
        VStack(alignment: .leading, spacing: 6) {
          Text("The functional anatomy of this Mac").font(.section.bold())
          Text(
        ...
        HStack(alignment: .top, spacing: 18) {
          VStack(alignment: .leading, spacing: 12) {
            Text(systemTitle(selectedSystem)).font(.section.bold())
  ...
  private var systemOverview: some View {
    VStack(alignment: .leading, spacing: 12) {
      Text("Systems working together").font(.subsection.bold())
      Text(
      ...
      .font(.secondary).foregroundStyle(.secondary)
        ...
        ForEach(systems, id: \.self) { system in
          Button {
            ...
            VStack(alignment: .leading, spacing: 8) {
              ...
              .font(.small.bold()).foregroundStyle(.blue)
            }
            .padding(14)
    ...
    Button {
      ...
      HStack(spacing: 14) {
        ...
        VStack(alignment: .leading, spacing: 4) {
          ...
          Text(node.path).font(.small.monospaced()).foregroundStyle(.tertiary).lineLimit(1)
        }
        Spacer()
  ...
  private var inspector: some View {
    ...
    return VStack(alignment: .leading, spacing: 14) {
      if let node {
        Label("ROLE IN THE SYSTEM", systemImage: node.symbol).font(.small.bold())
          .foregroundStyle(.blue)
        Text(node.label).font(.section.bold())
        Text(node.purpose)
        PathActionMenu(path: node.path)
        Divider()
        Text("Your data riding here").font(.rowTitle)
