...
struct InspectorView: View {
  ...
  private func entityView(_ entity: Entity) -> some View {
    ...
    return VStack(alignment: .leading, spacing: 18) {
      ...
      Text(entity.name).font(.heading.bold())
      if entity.type != .application || isDisplayGroup {
        Text(entity.summary).font(.subsection)
      }
      if isDisplayGroup {
    ...
    return Group {
      if !paths.isEmpty || manager == "Homebrew" {
        VStack(alignment: .leading, spacing: 10) {
          Text("Explore further").font(.rowTitle)
  ...
  private func relationshipView(_ relationship: Relationship) -> some View {
    VStack(alignment: .leading, spacing: 18) {
      ...
      Text(relationship.type.rawValue.capitalized).font(.heading.bold())
      HStack {
      ...
      .font(.rowTitle)
      Text(relationship.explanation).font(.subsection)
      Label(
