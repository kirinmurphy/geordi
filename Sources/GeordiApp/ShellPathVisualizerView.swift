    ...
    ScrollView {
      VStack(alignment: .leading, spacing: 18) {
        VStack(alignment: .leading, spacing: 6) {
          Text("Shell & PATH Visualizer").font(.display.bold())
          GlossaryAwareText(
              ...
              ForEach(analysis.operations) { operation in
                HStack(alignment: .top, spacing: 10) {
                  ...
                  VStack(alignment: .leading, spacing: 2) {
                    Text(operation.kind.rawValue.capitalized).font(.secondary.bold())
                    Text(operation.value).font(.small.monospaced())
                  }
                }
                .padding(9)
