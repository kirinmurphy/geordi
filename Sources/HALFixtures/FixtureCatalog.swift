...
public enum FixtureCatalog {
  ...
  public static let helperRichApplication = SystemGraph(
    metadata: FixtureMetadata(
      id: "helper-rich-application",
      name: "Helper-rich application",
      summary:
        "An editor with child processes, persistent login helper, caches, and shared support."
    ),
    entities: [
      entity(
        "app.forge", .application, "Forge Studio",
        "A developer editor with several supporting components.",
        [
          Detail("Origin", "Signed disk image"),
          Detail("Publisher", "Example Tools"),
          Detail("State", "Installed · Approved · Running"),
        ]),
      entity("process.forge", .process, "Forge Studio process", "The editor's main process."),
      entity(
        "process.renderer", .process, "Forge renderer",
        "Displays editor content in an isolated process."),
      entity("process.indexer", .process, "Forge indexer", "Indexes the open workspace."),
      entity(
        "persistence.forge", .persistence, "Forge login helper",
        "Starts Forge background services after sign-in."),
      entity("file.forge-cache", .file, "Forge cache", "Rebuildable editor cache."),
      entity("file.forge-log", .file, "Forge logs", "Diagnostic logs written by Forge."),
      entity(
        "file.shared-tools", .file, "Developer tools support",
        "A shared folder used by Forge and other tools."),
      entity("resource.forge", .resource, "Forge activity", "Combined editor resource use."),
    ],
    relationships: [
      link(
        "forge-launches", "app.forge", "process.forge", .launches, .confirmed,
        "Forge Studio launched its main process.",
        [
          evidence(
            "forge-main-observation", .observed, "The application and process identities match.",
            "Synthetic process observation")
        ]),
      link(
        "forge-renderer", "process.forge", "process.renderer", .launches, .confirmed,
        "The main Forge process launched its renderer.",
        [
          evidence(
            "forge-parent-renderer", .observed, "The renderer records Forge as its parent process.",
            "Synthetic process tree")
        ]),
      link(
        "forge-indexer", "process.forge", "process.indexer", .launches, .confirmed,
        "The main Forge process launched its indexer.",
        [
          evidence(
            "forge-parent-indexer", .observed, "The indexer records Forge as its parent process.",
            "Synthetic process tree")
        ]),
      link(
        "forge-persists", "app.forge", "persistence.forge", .persistsThrough, .high,
        "Forge appears configured to start a helper after sign-in.",
        [
          evidence(
            "forge-team-match", .observed, "The helper and application share a signing identity.",
            "Synthetic signing observation"),
          evidence(
            "forge-helper-rule", .inferred, "The helper label and bundle identity match Forge.",
            "Persistence resolver v1"),
        ]),
      link(
        "forge-cache", "process.renderer", "file.forge-cache", .readsWrites, .high,
        "The renderer is the likely active user of this cache.",
        [
          evidence(
            "forge-cache-event", .observed, "Renderer activity and cache writes overlap.",
            "Synthetic file event")
        ]),
      link(
        "forge-log", "process.indexer", "file.forge-log", .readsWrites, .confirmed,
        "The indexer writes diagnostic messages to these logs.",
        [
          evidence(
            "forge-log-writer", .observed, "The synthetic write event identifies the indexer.",
            "Synthetic file event")
        ]),
      link(
        "forge-shared", "app.forge", "file.shared-tools", .shares, .probable,
        "Forge uses this folder, but it is not the only plausible owner.",
        [
          evidence(
            "forge-shared-use", .observed, "Forge accessed the support folder.",
            "Synthetic file event"),
          evidence(
            "forge-shared-rule", .inferred,
            "Other developer tools also use this conventional path.", "Ownership resolver v1"),
        ]),
      link(
        "forge-resource", "process.indexer", "resource.forge", .consumes, .confirmed,
        "The indexer contributes most of Forge's current activity.",
        [
          evidence(
            "forge-resource-sample", .observed, "The indexer leads the sampled CPU contribution.",
            "Synthetic resource sample")
        ]),
    ]
  )
